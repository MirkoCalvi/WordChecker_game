...
int main(){
	...
	nuova_partita(match, vincoli);//inizio a rimpire i dizionario	
...
void nuova_partita(gamePointer partita, vincoliPointer *vincoli){
	...
	//*******************************************inizializing vincoli *****************************************************************
	gameON=true;
	free_vincoli(vincoli);

	memset(giuste, '\0', l);//---------------------------------------------------------------------------------------------------
	
	//*******************************************inizializing tree to nuova partita *****************************************************************
	...
	while(gameON){ //un pò brutale-----------------------------------------------------------------------------
		...
		if(c=='+'){//lets see wich command I have to do
			c=getc(stdin);
			...
			//+stampa_filtrate, in here I also have to do a scrematura
			if(c=='s'){
//				printf("\n_plausible_stampa:%d\n", conteggio_plausibili);
			...
			if(sameWord(parola, partita->RefWord)) { //la parola letta ? giusta
				...
				if(searchWord(dict, parola)){
					...
					for(i=0; i<l; i+=1){
						if(outWord[i]=='/'){//devo ferificare se in parola[] ci sono altre parola[i]; se è in eccesso sarà sicuramente dentro presenti, altrimenti sarà per forza assente		
							spot=letters_in_ascii(parola[i]);
	...
	for(k=0; k<DIM; k+=1){
		...
		if(valid[k]){
			...
			while( r<l && pass){//faccio scorrere giuste[]
				if(giuste[r]!='\0' && giuste[r]!=word[r]) pass=false;//non c'è la lettera giusta in quel punto	
				r+=1;
			}
				...
				while( r<ALPHA && pass){//faccio scorrere vincoli-lettere[]
					...
					if(vincoli[r]!=NULL){
						...
						if( vincoli[r]->n_pres!=-1 && pass){
							...
							if(q==l) pass=false; //significa che la lettera vincoli->lettere[r] non è presente nella parola quando invece dovrebbe esserlo, deve esserci ma non la trovo
							...
							while( p<l && pass){
								if(word[p]==lt){
									//verifico la "matrice"
									if(vincoli[r]->posix[p]==false) pass=false; //so che in quel punto non deve esseri quella lettera quindi se c'è non va mica bene	
									n+=1;
								}
								p+=1;
							}				
...
bool check_plausible(char *word, vincoliPointer *vincoli){
	...
	for(r=0; r<l; r+=1){//faccio scorrere giuste[]
		if(giuste[r]!='\0' && giuste[r]!=word[r]) return false;//non c'è la lettera giusta in quel punto	
	}
	...
	for(r=0; r<ALPHA; r+=1){//faccio scorrere vincoli-lettere[]
		...
		if(vincoli[r]!=NULL){
			...
			if( vincoli[r]->n_pres!=-1){
				//arrivato a questo punto la lettera[r] deve per forza essere presente, devo verificare quante volte e se rispetta i parametri
				if(q==l) return false; //significa che la lettera vincoli->lettere[r] non è presente nella parola quando invece dovrebbe esserlo, deve esserci ma non la trovo
				...
				for(p=0; p<l; p+=1){
					if(word[p]==lt){
						//verifico la "matrice"
						if(vincoli[r]->posix[p]==false) return false; //so che in quel punto non deve esseri quella lettera quindi se c'è non va mica bene	
						n+=1;
					}
				}				
